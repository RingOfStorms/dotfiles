          ...
          default = pkgs.mkShell {
            packages = [
              ...
              (pkgs.writeShellScriptBin "deploy_i001" ''
                NIX_SSHOPTS="-i /run/agenix/nix2nix" nixos-rebuild --flake $(git rev-parse --show-toplevel)'/hosts/i001#i001' --target-host root@10.12.14.119 --use-substitutes --no-reexec switch
              '')
              (pkgs.writeShellScriptBin "deploy_h002" ''
                NIX_SSHOPTS="-i /run/agenix/nix2nix" nixos-rebuild --flake $(git rev-parse --show-toplevel)'/hosts/h002#h002' --target-host root@10.12.14.183 --use-substitutes --no-reexec switch
              '')
              (pkgs.writeShellScriptBin "deploy_h003" ''
                NIX_SSHOPTS="-i /run/agenix/nix2nix" nixos-rebuild --flake $(git rev-parse --show-toplevel)'/hosts/h003#h003' --target-host root@10.12.14.1 --use-substitutes --no-reexec switch
              '')
              (pkgs.writeShellScriptBin "deploy_juni" ''
                NIX_SSHOPTS="-i /run/agenix/nix2nix" nixos-rebuild --flake $(git rev-parse --show-toplevel)'/hosts/juni#juni' --target-host josh@10.12.14.172 --use-substitutes --no-reexec switch
              '')
            ];
          };
        }
