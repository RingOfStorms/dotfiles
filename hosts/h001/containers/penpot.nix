...
let
  ...
  # systemd EnvironmentFile — `KEY=VALUE\n`. Loaded by the podman
  # backend/exporter units so PENPOT_SECRET_KEY arrives as a real env
  # variable inside the container without touching the nix store.
  secretKeyEnvFile = "${c.dataDir}/secret-key.env";

  # Common podman options — join the shared user-defined network so
  # container-name DNS works between them.
  netOpts = [ "--network=${network}" ];
...
in
{
  ...
  systemd.services = {
    ...
    # `secrets.token_urlsafe(64)` is exactly that. Persisted as a
    # systemd EnvironmentFile so it survives reboots and is loaded
    # straight into the backend/exporter container env.
    penpot-secret-key = {
  ...
  # Backend + exporter additionally load PENPOT_SECRET_KEY from the env
  # file generated by penpot-secret-key.service. systemd merges these
  # serviceConfig blocks with the ones synthesized by the oci-containers
  # module, so we don't lose the ExecStart.
  // {
    podman-penpot-backend.serviceConfig.EnvironmentFile = secretKeyEnvFile;
    podman-penpot-exporter.serviceConfig.EnvironmentFile = secretKeyEnvFile;
  };
  ...
  virtualisation.oci-containers.containers = {
    ...
    penpot-backend = {
      ...
      # PENPOT_SECRET_KEY is injected via systemd EnvironmentFile on the
      # podman-penpot-backend.service unit below — it does NOT appear in
      # the `environment` set here (which would land in the nix store).
      environment = {
    ...
    penpot-exporter = {
      ...
      # See penpot-backend re: PENPOT_SECRET_KEY (EnvironmentFile below).
      environment = {
